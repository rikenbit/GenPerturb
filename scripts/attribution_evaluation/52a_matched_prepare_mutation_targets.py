...
def parse_args():
    ...
    p.add_argument("--study", required=True,
                   choices=[k for k in STUDY_CONFIGS.keys() if k.endswith("_matched")])
    return p.parse_args()
...
def create_seqlet_metadata(matched):
    meta = matched.copy()
...
def deduplicate_sites(meta):
    site_cols = ["chromosome", "core_genomic_start", "core_genomic_end", "gene"]
    dedup = (
        meta.dropna(subset=["core_genomic_start", "core_genomic_end"])
        .drop_duplicates(subset=site_cols)
    )
    targets = dedup[["chromosome", "core_genomic_start", "core_genomic_end", "gene"]].copy()
    targets = targets.rename(columns={
        "core_genomic_start": "mut_start",
        "core_genomic_end": "mut_end",
    })
    targets["mut_start"] = targets["mut_start"].astype(int)
    targets["mut_end"] = targets["mut_end"].astype(int)
    targets["mut_length"] = targets["mut_end"] - targets["mut_start"]
    targets["mutation_type"] = "seqlet_mut"
    targets["site_id"] = (
        targets["chromosome"] + ":" + targets["mut_start"].astype(str)
        + "-" + targets["mut_end"].astype(str) + "__" + targets["gene"]
    )
...
def main():
    ...
    print("=" * 60)
    print(f"Preparing MATCHED mutation targets for {args.study}")
    ...
    print("\n[Step 2] Creating seqlet metadata...")
    meta = create_seqlet_metadata(matched)

    print("\n[Step 3] Deduplicating mutation sites...")
    targets = deduplicate_sites(meta)

    print("\n[Step 4] Finding negative control regions...")
    ...
    print("\n[Step 5] Extracting seqlet-level attribution...")
