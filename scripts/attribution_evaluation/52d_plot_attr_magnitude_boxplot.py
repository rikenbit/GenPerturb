...
def parse_args():
    ...
    p.add_argument("--study", required=True,
                   choices=list(STUDY_CONFIGS.keys()) + ["all"])
    return p.parse_args()
...
def plot_one_study(study, figures_dir, tables_dir):
    ...
    plt.tight_layout()
    svg_path = f"{figures_dir}/attr_magnitude_grouped_source_only_boxplot.svg"
    png_path = f"{figures_dir}/attr_magnitude_grouped_source_only_boxplot.png"
    ...
    counts_path = f"{tables_dir}/attr_magnitude_group_counts.tsv"
...
def main():
    args = parse_args()
    studies = list(STUDY_CONFIGS.keys()) if args.study == "all" else [args.study]
