...
martin_datasets = [("data/adata/MartinRufino2025.h5ad", "data/adata/MartinRufino2025")]

for input_path, prefix in martin_datasets:
    if not os.path.exists(input_path):
        print(f"  Skipping {input_path} (not found)")
        continue
    print(f"  Processing {input_path}...")
    adata = sc.read_h5ad(input_path)
    adata.obs["gene"] = adata.obs["target"]
    adata.var["ensembl_id"] = adata.var["gene_ids"]
    adata_mix = adata.copy()
    adata_mix = process_and_write_data(adata_mix, prefix, control_mr, perturbation_type='KD')
    filter_adata(adata_mix, adata, prefix)
    del adata, adata_mix
    gc.collect()
...
### Wu2024 (GSE277747) ###
wu_datasets = [("data/adata/Wu2024.h5ad", "data/adata/Wu2024")]
for input_path, prefix in wu_datasets:
    if not os.path.exists(input_path):
        print(f"  Skipping {input_path} (not found)")
        continue
    print(f"  Processing {input_path}...")
    adata = sc.read_h5ad(input_path)
    adata.obs["gene"] = adata.obs["target"]
    # Mixscape's rank_genes_groups chokes on singleton groups.
    vc = adata.obs["gene"].value_counts()
    adata = adata[adata.obs["gene"].isin(vc[vc >= 3].index)].copy()
    adata.var["ensembl_id"] = adata.var["ensembl_gene"].astype(str)
    adata_mix = adata.copy()
    adata_mix = process_and_write_data(adata_mix, prefix, "NT", perturbation_type='KD')
    filter_adata(adata_mix, adata, prefix)
    del adata, adata_mix
    gc.collect()
...
metz_datasets = [("data/adata/Metzner2025.h5ad", "data/adata/Metzner2025")]
MIN_CELLS_PER_TARGET = 3
for input_path, prefix in metz_datasets:
    if not os.path.exists(input_path):
        print(f"  Skipping {input_path} (not found)")
        continue
    print(f"  Processing {input_path}...")
    ...
    adata.obs["gene"] = adata.obs["target"]
    vc = adata.obs["gene"].value_counts()
    keep_targets = set(vc[vc >= MIN_CELLS_PER_TARGET].index)
    adata = adata[adata.obs["gene"].isin(keep_targets)].copy()
...
### Shevade2025 (GSE288996) ###
shev_input = "data/adata/Shevade2025_K562_DMSO.h5ad"
shev_prefix = "data/adata/Shevade2025_K562_DMSO"
if os.path.exists(shev_input):
    print(f"  Processing {shev_input} (bulk-only, no Mixscape)...")
    adata = sc.read_h5ad(shev_input)
    sc.pp.normalize_total(adata, target_sum=1e4)
    sc.pp.log1p(adata)
    adata.obs["gene"] = adata.obs["target"]
    adata.obs["perturbation"] = "NT"
    adata.obs["mixscape_class_global"] = "NT"
    adata.X = sp.sparse.csr_matrix(adata.X)
    adata.write(shev_prefix + "_mixscape.h5ad")
    adata.write(shev_prefix + "_mixscape_exnp.h5ad")
