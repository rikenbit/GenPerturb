...
def _is_alphagenome(pretrained):
    return pretrained is not None and (
        pretrained == "alphagenome" or pretrained.startswith("alphagenome_fold_")
    )
...
class JointDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.alphagenome_input = _is_alphagenome(pretrained)
        ...
        if self.hdf5 is not None:
            ...
            self.fasta_dataset = self.create_seq(bed, fasta, context_length, lambda df: df.filter(pol.col("column_7") == datasplit))
            self.target = torch.tensor(df.query('training == @datasplit').drop(labels="training", axis=1).astype("float32").values)

    def create_seq(self, bed, fasta, context_length, filter_df_fn):
    ...
    def __getitem__(self, idx):
        if self.hdf5 is not None:
            ...
            if self.alphagenome_input:
                seq = alphagenome_indices_to_one_hot(
                    seq,
                    sequence_key=self.fasta_dataset.get_interval_key(idx),
                )
...
class JointDataLoader(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        #return DataLoader(self.dfs[0], batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False)
...
class JointDatasetPred(torch.utils.data.Dataset):
    ...
    def __len__(self):
        if self.hdf5 is not None:
            return self.hdf5_dataset.file['embedding'].shape[0]
        else:
            return len(self.fasta_dataset)
