#!/bin/bash
# ==============================================================================
# Fig 3e / S10a: Captum for paper motif-discovery top-200 route
# ==============================================================================
...
TASKS_DIR="attribution/fig3e_top200_tasks"
TASKS_FILE="${TASKS_DIR}/tasks_${STUDY_FULL}.txt"
DONE_DIR="${TASKS_DIR}/done/${STUDY_FULL}"
mkdir -p "$TASKS_DIR" "$DONE_DIR"

echo "Fig 3e / S10a Captum top-200 — study=${STUDY_FULL} mode=${MODE} target=${TARGET}"

# --- 1. generate task list ---------------------------------------------------
pipeline_activate_conda alphagenome
GENERATE_ARGS=(fig3e_top200 "$STUDY" "$MODEL" --mode "$MODE" --target "$TARGET" --output "$TASKS_FILE")
...
PENDING_IDS=()
while IFS=$'\t' read -r TASK_ID ROUTE STUDY_FIELD SUFFIX_FIELD MODEL_FIELD MODE_FIELD TARGET_FIELD PERT_FIELD; do
    SAFE_PERT="${PERT_FIELD//\//_}"
    RAW_H5="attribution/${STUDY_FIELD}__${SUFFIX_FIELD}/${SAFE_PERT}/${SAFE_PERT}_raw_attribution.h5"
...
if [ "${#PENDING_IDS[@]}" -eq 0 ]; then
    echo "All Fig 3e / S10a top-200 Captum raw H5 files already exist; no SLURM jobs submitted."
...
SBATCH_ARGS=(
    ...
    --job-name="fig3e_top200_${STUDY%%_*}"
    --output="log/fig3e_top200_%A_%a.out"
    --error="log/fig3e_top200_%A_%a.err"
)
...
echo "Submitted Fig 3e top-200 Captum array: $ARRAY_JOBID"
...
PEAK_JOBID=$(sbatch --parsable \
    ...
    --cpus-per-task=4 --nice=100 \
    --job-name="fig3e_top200_peak_${STUDY%%_*}" \
    --output="log/fig3e_top200_peak_%j.out" \
    --error="log/fig3e_top200_peak_%j.err" \
    --wrap="bash pipeline/fig3_attribution/12_peak_call.sh '$STUDY' '$MODEL' '$STUDY_SUFFIX'")
